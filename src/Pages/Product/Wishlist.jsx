  ...
  return (
    <>
      ...
      <div className="container mx-auto p-4 h-full">
        <h2 className="text-3xl font-bold mb-6 text-center lg:pt-0 pt-20">My Wishlist</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {wishlist.length === 0 ? (
            <p className="text-center col-span-full text-gray-600">Your wishlist is empty.</p>
          ) : (
            wishlist.map((product) => (
              <div key={product._id} className="relative bg-white border border-gray-200 rounded-lg shadow-lg overflow-hidden transition-transform transform hover:scale-105">
                <button
