...
const OrderHistory = () => {
  ...
  useEffect(() => {
    if (user && user.email) {
      axios.get(`/api/order/user/${user.email}`)
        .then(response => {
  ...
  return (
    <div className="container mx-auto p-6 bg-gray-100 min-h-screen">
      <h1 className="text-3xl font-bold mb-6 text-center text-gray-800 lg:pt-0 md:pt-10 pt-10">Order History</h1>
      {orders.length > 0 ? (
              ...
              <div className="p-6">
                ...
                <div className="order-products">
                  {order.products && order.products.length > 0 ? (
                    order.products.map((item, idx) => (
                      <div key={item.product._id + idx} className="border-t border-gray-200 pt-4 mt-4">
                        <Link to={`/product/${item.product._id}`}>
                          ...
                          <p className="text-sm text-gray-600">Original Price: {formatCurrency(item.product.price)}</p>
                          {item.product.discountPercentage > 0 && (
                            <p className="text-sm text-green-500">
                              Price After Discount: {formatCurrency(calculatePriceAfterDiscount(item.product.price, item.product.discountPercentage))}
                            </p>
                    ...
                    <p className="text-center text-lg text-gray-500">No products found in this order.</p>
                  )}
                </div>
              </div>
            </div>
