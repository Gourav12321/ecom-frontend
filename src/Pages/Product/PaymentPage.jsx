...
const PaymentPage = () => {
  ...
  const handlePayment = async () => {
    ...
    try {
      // Create order
      const { data: orderData } = await apiClient.post("/api/order/add", {
      ...
      const { data: paymentResponse } = await apiClient.post(
        "/api/order/payment",
        {
          totalAmount: orderSummary.totalAmount,
      ...
      await apiClient.post("/api/order/update-status", {
        paymentIntentId: paymentIntent.id,
      });
      await apiClient.post("/api/cart/clear", { email: user.email });

      toast.success("Payment Successful!");

      // Generate and open PDF
      const { data: pdfResponse } = await apiClient.post(
        "/api/order/generate-pdf",
        {
          user,
      ...
      <div className="payment-page container mx-auto p-4 md:p-8 bg-gray-100 h-full flex flex-col justify-center items-center">
        ...
        <div className="mt-8 bg-white p-6 md:p-10 rounded-lg shadow-lg w-full max-w-md">
          ...
          <div className=" my-6 w-full">
            <p className="text-center font-extrabold text-xl mb-3">
