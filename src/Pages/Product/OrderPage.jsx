    ...
    if (user && user.email) {
      const fetchData = async () => {
        try {
          ...
          const discountedAmount = summaryResponse.data.totalAmount * (discountPercentage / 100);
          const totalAfterDiscount = summaryResponse.data.totalAmount - discountedAmount;
  ...
  return (
    <>
      ...
      <div className="order-page container mx-auto p-4 md:p-8 h-full">
              ...
              addresses.map((address, idx) => (
                <div
                  ...
                  className={`address-item p-4 border rounded-lg cursor-pointer transition-colors ${
                    selectedAddress === address ? 'bg-blue-100 border-blue-500' : 'bg-white'
                  }`}
                >
                  <p className="font-semibold">{address.phoneNumber}</p>
                  <p className="font-semibold">{address.houseNo}, {address.street}, {address.city}</p>
                  <p className="text-sm text-gray-600">{address.district}, {address.state}, {address.pincode}</p>
        ...
        <h2 className="text-2xl md:text-3xl font-bold mt-8 mb-4">Order Summary</h2>
        {orderSummary ? (
          <div className="order-summary bg-gray-100 p-4 rounded-lg">
            {orderSummary.products.map((item, idx) => (
              <div key={idx} className="order-item flex justify-between py-2 border-b last:border-b-0">
                <p className="font-semibold">{item.product.title} (x{item.quantity})</p>
                <p className="font-bold">Rs. {item.price.toFixed(2)}</p>
              </div>
            ))}
            <p className="text-xl font-bold mt-4">
              Discounted Total: Rs. {discountedTotal.toFixed(2)} {orderSummary.discountPercentage > 0 && <span className="text-sm text-gray-600">({orderSummary.discountPercentage}% off)</span>}
            </p>
          </div>
        ) : (
