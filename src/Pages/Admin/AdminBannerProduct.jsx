...
function AdminBannerProduct() {
  ...
  const handleSave = async (categoryId, imageUrl) => {
    try {
      ...
      const filteredCategories = updatedResponse.data.categories.filter(
        (photo) => photo.bannerPhoto !== null
      );
      setPhotoData(filteredCategories);
    ...
    <div className="flex flex-col mt-14 p-4 sm:p-6 md:p-8">
      ...
      <div className="flex flex-wrap gap-5 justify-center">
        {photoData.map((photo, index) => (
          <div
            key={index}
            className="w-full sm:w-[48%] md:w-[45%] lg:w-[30%] h-[170px] relative"
          >
            <Link
              to={`/product-category?category=${photo._id}`}
              className="flex w-full h-full z-10"
            >
              <img
                src={photo.bannerPhoto}
                alt={photo.name}
